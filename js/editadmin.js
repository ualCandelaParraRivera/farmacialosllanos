        ...
        $.ajax({
            ...
            .done(function(data) {
                ...
                if (!data.success) {
                    ...
                    if (data.errors.intro) {
                        $('#intro').addClass('is-invalid'); // add the error class to show red input
                        $('#intro-group').append('<div class="help-block">' + data.errors.intro + '</div>'); // add the actual error message under our input
                    }
                    if (data.errors.profile) {
                        $('#profile').addClass('is-invalid'); // add the error class to show red input
                        $('#profile-group').append('<div class="help-block">' + data.errors.profile + '</div>'); // add the actual error message under our input
                    }
                    $('form#datospersonales').append('<div class="mt-3 alert alert-danger">' + data.message + '</div>');

                } else {

                    window.location.replace("admineditadmins?guiduser="+data.message);
                }
            });
