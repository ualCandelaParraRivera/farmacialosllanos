...
if(isset($_POST['create'])){
    ...
    if(!isset($_POST['intro']) || empty($_POST['intro'])){
        $errors['intro'] = "Debes introducir una introducción (p.e. cargo en la empresa)";
    }else{
        $intro = $_POST['intro'];
    }

    if(!isset($_POST['profile']) || empty($_POST['profile'])){
        $errors['profile'] = "Debes introducir la información del perfil";
    }else{
    ...
    if (!empty($errors)) {
        ...
        if($newuser==1){
            $query = "INSERT INTO `user` (`firstName`, `middleName`, `lastName`, `mobile`, `email`, `password`, `intro`, `profile`, `image`, `admin`, `vendor`, `registeredAt`, `lastLogin`, `isdeleted`, `isvalid`, `guiduser`) VALUES
            (?, ?, ?, ?, ?, sha1(UUID()), ?, ?, 'team-1.jpg', 1, 0, NOW(), NOW(), 0, 1, UUID())";
            $db->prepare($query, array($firstname, $middlename, $lastname, $mobile, $email, $intro, $profile));
            $id = $db->lastID();
            ...
            $row = mysqli_fetch_array($res);
            $id = $row['id'];
            $query = "UPDATE `user` SET firstname = ?, middlename = ?, lastname = ?, mobile = ?, email = ?, intro = ?, profile = ? WHERE id = ?";
            $db->prepare($query, array($firstname, $middlename, $lastname, $mobile, $email, $intro, $profile, $id));
        }
        $data['success'] = true;
        $data['errors']  = $errors;
 ...
 echo json_encode($data);
    ...
    if(!isset($_POST['billfirstname']) || empty($_POST['billfirstname'])){
        $errors['billfirstname'] = "Debes introducir el nombre del administrador";
    }else{
        $billfirstname = $_POST['billfirstname'];
    }

    if(!isset($_POST['billmiddlename']) || empty($_POST['billmiddlename'])){
        $errors['billmiddlename'] = "Debes introducir el primer apellido del administrador";
    }else{
    ...
    if(!isset($_POST['billmobile']) || empty($_POST['billmobile'])){
        $errors['billmobile'] = "Debes introducir el teléfono del administrador";
    ...
    if(!isset($_POST['shipfirstname']) || empty($_POST['shipfirstname'])){
        $errors['shipfirstname'] = "Debes introducir el nombre del administrador";
    }else{
        $shipfirstname = $_POST['shipfirstname'];
    }

    if(!isset($_POST['shipmiddlename']) || empty($_POST['shipmiddlename'])){
        $errors['shipmiddlename'] = "Debes introducir el primer apellido del administrador";
    }else{
    ...
    if(!isset($_POST['shipmobile']) || empty($_POST['shipmobile'])){
        $errors['shipmobile'] = "Debes introducir el teléfono del administrador";
